...
if IS_REPLIT:
    ...
    class ReplitDBHandler(DatabaseHandler):
        def get(self, key, default=None):
            return replit_db.get(str(key), default)
        def set(self, key, value):
            replit_db[str(key)] = value
        def delete(self, key):
            key_str = str(key)
            if key_str in replit_db:
                del replit_db[key_str]
                return True
            return False
        def all(self):
            return {key: replit_db[key] for key in replit_db.keys()}
        def prefix(self, p_str: str = ""):
            return tuple(key for key in replit_db.keys() if key.startswith(p_str))
...
# --- MongoDB用の処理 ---
    ...
    class MongoDBHandler(DatabaseHandler):
        ...
        def get(self, key, default=None):
            ...
            # ★★★ 修正点: valueではなく、データ本体(data)を返す ★★★
            return document.get("data") if document else default

        def set(self, key, value):
            if not self.client: return
            # ★★★ 修正点: valueをdataキーの中に格納して保存 ★★★
